...
const managedAccountRoleOptions = backendAccountRoleOptions;
...
function AccountEditor({ mode, account, hotels, onClose, onSubmit }: { mode: "create" | "edit"; account: AccountRecord | null; hotels: HotelSummary[]; onClose: () => void; onSubmit: (values: AccountFormValues) => Promise<void> }) {
  ...
  return <Modal open title={mode === "create" ? "新增后台账号" : "编辑后台账号"} destroyOnHidden footer={null} onCancel={onClose} width={560}>
    <Form key={`${mode}-${account?.id ?? "new"}`} layout="vertical" initialValues={initialValues} onFinish={onSubmit}>
      {mode === "create" ? <><Form.Item label="用户名" name="username" rules={[{ required: true, message: "请输入用户名" }]}><Input autoComplete="off" /></Form.Item><Form.Item label="初始密码" name="password" rules={[{ required: true, min: 6, message: "密码至少 6 位" }]}><Input.Password autoComplete="new-password" /></Form.Item></> : <Form.Item label="用户名"><Input value={account?.username} disabled /></Form.Item>}
...
export function AccountsClient() {
  ...
  function resetPassword(account: AccountRecord) { modal.confirm({ title: "确认重置密码", content: `确认将账号“${account.username}”的密码重置为系统自动生成的 6 位数字密码？`, okText: "确认重置", cancelText: "取消", onOk: async () => { try { const result = await request<{ id: string; newPassword: string }>(`/api/accounts/${account.id}/reset-password`, { method: "POST" }); modal.success({ title: "密码已重置", content: <Descriptions bordered column={1} size="small"><Descriptions.Item label="账号">{account.username}</Descriptions.Item><Descriptions.Item label="新密码">{result.newPassword}</Descriptions.Item></Descriptions> }); } catch (error) { message.error(error instanceof Error ? error.message : "重置失败"); throw error; } } }); }
